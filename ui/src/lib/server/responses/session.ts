...
export class PostgresMemorySession implements Session {
  ...
  async getItems(limit?: number): Promise<AgentInputItem[]> {
    ...
    const loadedItems = [
      ...
      ...items.functionCalls.map(
        ({ eventIdx, name, callId, status, arguments: args, providerData }) => ({
          eventIdx,
          providerData,
          type: 'function_call',
          name,
  ...
  async addItems(items: AgentInputItem[]): Promise<void> {
    ...
    console.log(`Adding items to memory session (${this.sessionId}): ${JSON.stringify(items)}`);
    await Promise.all(
      items.map(async (item) => {
        switch (item.type) {
          case 'message': {
            const [{ messageId }] = await getDb()
              .insert(T.ResponsesApiMessage)
              .values({
            ...
            if (item.role === 'user') {
              ...
              return getDb()
                .insert(T.ResponsesApiMessageContent)
                .values(
                  item.content.map(({ type, text }) => {
                    if (type !== 'output_text') {
                      throw new Error(
                        `Attempting to save unknown message type: ${JSON.stringify(item)}`,
                      );
                    }
          ...
          case 'function_call_result': {
            return getDb()
              .insert(T.ResponsesApiFunctionResult)
              .values({
                chatId: this.sessionId,
                eventIdx: tick++,
                name: item.name,
